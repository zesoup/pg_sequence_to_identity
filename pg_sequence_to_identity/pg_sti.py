...
def _fix_sequences():
    ...
    with connection.cursor(cursor_factory=psycopg2.extras.DictCursor) as curs:
        for row in _get_affected(with_identity=True, with_sequences=True):
            curs.execute("""SELECT coalesce( max( "%s" )+1, 1 ), pg_get_serial_sequence( '%s', '%s' ) FROM "%s"."%s" """
                % (row['column'],
...
$F$
    ...
    begin
        ...
        alter table only "%s"."%s" alter "%s" set default null;

        DROP SEQUENCE %s;
        -- It seems wise to keep the last counter of the sequence. For this reason we're logging out to
        -- postgresql's logfile.

        alter table only "%s"."%s" alter COLUMN "%s" ADD GENERATED by default as identity ;
